export function init(enviromentOrigin) {
  ...
  $("#login_not_ok").click(function () {
    window.location.href =
      enviromentOrigin + "/login?nextRequest=%22/cdm/redirect%22";
  });

  // Initial state update
  updateState(enviromentOrigin);
...
function updateState(enviromentOrigin) {
  // Check KBase token is present and valid
  checkKBaseCookie(enviromentOrigin).then((username) => {
    if (username) {
...
function checkKBaseCookie(enviromentOrigin) {
  const tokenCookie = getCookie("kbase_session");
