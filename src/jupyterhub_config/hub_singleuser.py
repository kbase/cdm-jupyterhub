...
class SingleUserEnvManager:
    ...
    def start_single_user_server(self):
        ...
        """
        self.log.info(
            f"Starting single-user server for {self.username} with environment: {self.environment}"
        )
        env_vars = [f"{key}={value}" for key, value in self.environment.items()]

        cmd = (
            ["sudo", "-E", "-u", self.username, "env"]
            + env_vars
    ...
    def _ensure_system_user(self):
        ...
        lock_file = os.path.join(
            tempfile.gettempdir(), f"user_creation_{self.username}.lock"
        )
                ...
                try:
                    ...
                    if self.groupname:
                        ...
                        try:
                            ...
                            self.log.info(
                                f"Group {self.groupname} does not exist, creating it."
                            )
                            subprocess.run(
                                ["sudo", "groupadd", self.groupname], check=True
                            )
    ...
    def _ensure_workspace_permission(self):
        ...
        subprocess.run(
            ["sudo", "chown", "-R", f"{self.username}:{group_name}", self.user_dir],
            check=True,
        )
